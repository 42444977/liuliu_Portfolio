...
- `js/main.js` — 渲染引擎。讀取 `works.js` 的 `SITE`/`WORKS` 並填入 DOM：網頁標題/頁首/頁尾、關於區塊（含聯絡方式）、作品牆、燈箱、分享按鈕。作品牆是等高格子網格：JS 只依序把作品塞進 `#works`，欄數與裁切都由 CSS 處理；燈箱永遠顯示完整原圖，裁切只影響縮圖牆。燈箱支援上一張/下一張/Esc/手機左右滑動/張數指示，關閉走 history 機制（開啟時 `pushState`，實際關閉只在 `popstate` handler 做，讓手機返回鍵關燈箱而非離開網站）。頁面載入後會逐張背景預載原圖（`preloadCache`），點開燈箱若原圖已就緒直接顯示，否則縮圖模糊頂著、原圖載完淡入。這個檔案是設定驅動的，本身沒有任何內容。
...
- `css/style.css` — 所有樣式都由 `:root` 中集中定義的 CSS 自訂屬性驅動（`--ink`、`--gray`、`--line`、`--bg`）。作品牆的版面全在這裡：`.gallery` 是 CSS grid（3/2/1 欄，斷點 900px/560px），`.work img` 用 `aspect-ratio: 3/4` + `object-fit: cover` 把不同尺寸的圖裁成等高格子，手機單欄改回 `aspect-ratio: auto` 不裁切；`main.js` 不管版面。燈箱用 opacity/visibility 過場（不是 display 切換），作品圖載入完成才由 `.is-loaded` 淡入。
